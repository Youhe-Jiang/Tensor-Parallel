...
class DpOnModel:
    ...
    def _build_dp_and_run(self, pp_deg, bsz):
        ...
        other_mem_cost = int(np.ceil(mem_cost_list[0]['other']))
        ...
        for i in range(pp_deg):
            ...
            if history_results[i] is not None:
                ...
                if i == 0:
                    if self.max_mem - other_mem_cost <= 0:
                        return np.inf, None, -1, np.inf, False, False
                    dp = DPAlg(self.max_mem - other_mem_cost, pp_stage_list[i], strategy_num)
                else:
                    dp = DPAlg(self.max_mem, pp_stage_list[i], strategy_num)
