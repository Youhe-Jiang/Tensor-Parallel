...
def wrap_data_parallel(module, dp_type = None, dp_group = None, gradient_as_bucket_view = True, broadcast_buffers = True, overlap = True, module_type='bert_enc', pp_device = None):
    if dp_type is None:
        return module
    elif dp_type == 0:
        ...
        if overlap:
            ...
            return DDP_apex(module, process_group = comm_group, delay_allreduce = True)
    elif dp_type == 1:
...
def wrap_modules_data_parallel(module_list, dp_types, dp_groups, module_types, overlap = True, pp_devices=None):
    ...
    # module_names = [module_name for module_name, _ in module_list.named_children()]
    # print(module_names)
    for i in range(len(module_list)):
        ...
        if 'embed' in module_types[i]:
            module_list[i] = wrap_data_parallel(module_list[i], dp_types[i], dp_groups[i], gradient_as_bucket_view = False,  
                    broadcast_buffers = False, overlap = overlap, module_type=module_types[i], pp_device = pp_device)
        else:
            module_list[i] = wrap_data_parallel(module_list[i], dp_types[i], dp_groups[i], overlap = overlap, module_type=module_types[i], pp_device = pp_device)
        # # Auto Wrap
        # module_list[i] = my_auto_wrap(module_list[i], dp_types[i], dp_groups[i])
    return module_list
