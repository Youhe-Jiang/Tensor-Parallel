...
class MemoryCostModelDist:
    def __init__(self,
        ...
        if 'fsdp' in self.strategy[-1].keys() and self.strategy[-1]['fsdp']:
            ...
            # # for zero-2
            # self.model_states_size  = 3/4*self.model_states_size * (1/self.dp_size + 0.03) + 1/4*self.model_states_size

            # if chunks > 1 and self.activation_size / chunks < 2 * self.parameter_size:
            #     self.model_states_size += 2 * self.parameter_size - self.activation_size / chunks
...
class TimeCostModelDist_with_overlap:
    def __init__(self,
        ...
        if self.pp_size > 1 and p2p_comm_coe_dict is not None:
            ...
            # if self.pp_size == 2:
            #     self.p2p_meg_size = self.p2p_meg_size / self.optimal_microbatch
    ...
    def gen_result(self):
        ...
        if self.fsdp:
            # forward_allgather_time = self.dp_message_size * (self.pp_size+self.optimal_microbatch-1)/self.pp_size * self.dc 
...
class TimeCostModelDist_with_overlap_hard_coded:
    def __init__(self,
            strategy,
            global_batch_size,
            parameter_size = 48,
        ...
        if 'fsdp' in strategy[-1].keys() and strategy[-1]['fsdp']:
            self.fsdp = True
        self.bs = global_batch_size
        # df_list = self.read_excel_list('./test_res.xlsx')
        # df_list = self.read_excel_list('./64gpus_2560_local_2.xlsx')
        # df_list = self.read_excel_list('./search_-vi-t_64/vit_64_32v2.xlsx')
        df_list = self.read_excel_list('./64gpus_2560_local_2.xlsx')
        # df_list = self.read_excel_list('./vit_64gpus_2560.xlsx')
        for d in df_list:
            if d['bsz'] == self.bs:
                self.df = d
                break
    ...
    def read_excel_list(self, path_file_name):  # Return 字典格式数据
            import pandas as pd
    ...
    def gen_result(self):
        # if self.fsdp:
        #     key = '%d%d%df'%(self.pp_size, self.tp_size, self.dp_size)
        # else:
        #     key = int('%d%d%d'%(self.pp_size, self.tp_size, self.dp_size))
        if self.fsdp:
            key = '%d %d %df'%(self.pp_size, self.tp_size, self.dp_size)
        else:
            key = '%d %d %d'%(self.pp_size, self.tp_size, self.dp_size)
        result = self.df[key]
        result = result / self.layer_num
        return result
